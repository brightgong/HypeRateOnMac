...
一款 macOS 菜单栏应用，用于实时显示 HypeRate 设备的心率数据。
...
- **实时心率显示**：通过 WebSocket 连接实时获取心率数据
...
1. 从 [Releases](https://github.com/brightgong/HypeRateOnMac/releases) 下载最新版本
2. 解压后将 `HypeRateOnMac.app` 拖入 `/Applications` 目录
3. 启动应用，输入你的 HypeRate 设备 ID 即可使用

> 💡 直接下载的版本已内置 API Key，无需额外配置
...
将构建产物中的 `HypeRateOnMac.app` 复制到 `/Applications` 目录。
